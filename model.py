...
class AAA(nn.Module):
    ...
    def __init__(self, embedding_matrix, opt):
        ...
        # pos_number = len(opt.tokenizer.vocab['pos']) #词典中pos标签的个数
        # pos_dim = opt.pos_dim #pos向量的维度
        # pos_pad = opt.tokenizer.vocab['pos'].pad_id  #需要进行padding的pos标签index        
        # relation_number = len(opt.tokenizer.vocab['deprel']) #词典中依赖关系的个数
        # relation_dim = opt.dep_dim #依赖关系的向量维度
        # relation_pad = opt.tokenizer.vocab['deprel'].pad_id  #需要进行padding的pos依赖关系index
        # hidden_dim = opt.hidden_dim #LSTM单元的隐状态维度

        # '''embedding layer'''
        # self.word_embedding = nn.Embedding.from_pretrained(torch.tensor(embedding_matrix, dtype=torch.float)) # 预训练的embedding层
        # self.pos_embedding = nn.Embedding(pos_number, pos_dim, padding_idx=pos_pad) #标签的embedding层
        # self.deprel_embedding = nn.Embedding(relation_number, relation_number, padding_idx=relation_pad) # 依赖关系的embedding层

        # '''parameters'''
        # self.pad_word = nn.Parameter(torch.zero(hidden_dim*2), requires_grad=False) # 对训练单词进行padding
        # self.pad_edge = nn.Parameter(torch.ones(1), requires_grad=False) 
        # self.ext_relation = nn.Parameter(torch.Tensor(relation_dim), requires_grad=False) # ???

        # '''模型神经网络层'''
        # self.rnn = DynamicLSTM(word_dim+pos_dim, hidden_dim, num_layers=1, batch_first=True, bidirectional=True, rnn_type='GRU') #Bi-GRU层
        # self.bilinear = nn.Bilinear(hidden_dim*4, relation_dim, 1, bias=True) #线性变换层，输出为1维向量 y = xA^T + b
        # self.fc_out = nn.Linear(hidden_dim*2, 3, bias=True) #全连接层 y = xA^T + b

        # self.embed_dropout = nn.Dropout(opt.embed_dropout) # embedding层的dropout
        # self.bilinear_dropout = nn.Dropout(opt.bilinear_dropout) #bilinear的dropout
        # self.fc_dropout = nn.Dropout(opt.fc_dropout) #全连接层的dropout

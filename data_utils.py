...
class MyDataset(Dataset):
    ...
    def __init__(self, fname, tokenizer):
        ...
        if os.path.exists(data_file):
            print(f"There exist data files, loading dataset: {data_file}")
            dataset = pickle.load(open(data_file, 'rb'))  # load dataset
        else:
            print('building dataset...')  # if there are no .dat feature file, then build it
    ...
    def _read_data(fname, tokenizer):
        ...
        ##### Cross-SA
        polarity_dict = {'positive': 0, 'negative': 1}

        fdata = json.load(open(fname, 'r', encoding='utf-8'))
        for data in fdata:
            for aspect in data['aspects']:
                start, end = int(aspect['from']), int(aspect['to']) # start是aspect开始的值，end是aspect结束的值
                ''' position indicator '''
                text_left = data['token'][0:start]  #aspect左边的上下文部分
                text_aspect = data['token'][start:end] # asepct词的部分
                ...
                gather_idx = [i+1 for i in range(len(text_left))] + [i+2+start for i in range(len(text_aspect))] +
                             [i+3+end for i in range(len(text_right))]
...
def build_embedding_matrix(vocab, dataset, word_dim=300):  # 构建词向量矩阵
